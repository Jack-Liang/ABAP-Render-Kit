...
| [`ui5-shell-preview.html`](ui5-shell-preview.html) | **UI5 启动壳真源（阶段②③）** | `zcl_ark_ui5_shell` 的浏览器可测真源：state→UI5 映射层（sap.m 工具栏/表格 + ECharts 分区 + 桥）。内置 mock 桥与页内自检（加载 10-20 秒后读 `#ark5_testlog` 四项判定；CDN 标签在文档末尾，自检等 UI5 就绪）。改壳请改本页后跑 `tools/gen_ui5_shell.mjs` 重新生成 ABAP 类 |
